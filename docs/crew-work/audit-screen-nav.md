...
**Gap:** The full `screen_ui.redraw()` (which shows track, page, play state, step positions) is unused in seamstress mode. All that info is invisible.
...
- Keyboard shortcuts for probability page (`Ctrl+P`) and scale page are missing (scale has no keyboard shortcut -- only grid nav x=15).
...
- Line 458 has comment `-- x=15: blank (no LED)` but it actually draws `NAV_META` (x=16) -- the comment references x=15 when it should reference x=16. This is cosmetic (code behavior is correct via the constant).
